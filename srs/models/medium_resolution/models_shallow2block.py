...
# Shallow Encoder with 2 blocks
class ShallowEncoder(nn.Module):
    def __init__(self, dropout_prob=0.15):
        super(ShallowEncoder, self).__init__()
        self.enc1 = EncoderBlock(in_channels_variable, 64, dropout_prob)
        self.enc2 = EncoderBlock(64, 128, dropout_prob)
        self.bridge = ConvBlock(128, 256, dropout_prob)

    def forward(self, x):
        s1, p1 = self.enc1(x)
        s2, p2 = self.enc2(p1)
        encoded = self.bridge(p2)
        return encoded, (s1, s2)
...
# Shallow Decoder for Autoencoder with 2 blocks
...
# Shallow U-Net with 2 blocks
class Unet(nn.Module):
    def __init__(self, num_classes=1, dropout_prob=0.15):
        super(Unet, self).__init__()
        self.encoder = ShallowEncoder(dropout_prob)
    ...
    def forward(self, x):
        encoded, (s1, s2) = self.encoder(x)
        x = self.dec2(encoded, s2)
        x = self.dec1(x, s1)
        x = self.final_conv(x)
        x = self.activation(x)
        return x
