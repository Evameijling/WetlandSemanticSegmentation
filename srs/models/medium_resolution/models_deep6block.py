...
class Encoder(nn.Module):
    def __init__(self, dropout_prob=0.25):
        ...
        self.enc5 = EncoderBlock(512, 1024, dropout_prob)
        self.enc6 = EncoderBlock(1024, 2048, dropout_prob)
        self.bridge = ConvBlock(2048, 4096, dropout_prob)
    ...
    def forward(self, x):
        ...
        s5, p5 = self.enc5(p4)
        s6, p6 = self.enc6(p5)
        encoded = self.bridge(p6)
        return encoded, (s1, s2, s3, s4, s5, s6)
...
# Decoder for Autoencoder with 6 blocks
class Decoder(nn.Module):
    def __init__(self):
        super(Decoder, self).__init__()
        self.dec6 = DecoderBlockForAutoencoder(4096, 2048)
        self.dec5 = DecoderBlockForAutoencoder(2048, 1024)
        self.dec4 = DecoderBlockForAutoencoder(1024, 512)
        self.dec3 = DecoderBlockForAutoencoder(512, 256)
        self.dec2 = DecoderBlockForAutoencoder(256, 128)
        self.dec1 = DecoderBlockForAutoencoder(128, 64)
...
class Unet(nn.Module):
    def __init__(self, num_classes=1, dropout_prob=0.25):
        ...
        self.dec6 = DecoderBlockForUnet(4096, 2048, dropout_prob)
        self.dec5 = DecoderBlockForUnet(2048, 1024, dropout_prob)
        self.dec4 = DecoderBlockForUnet(1024, 512, dropout_prob)
        self.dec3 = DecoderBlockForUnet(512, 256, dropout_prob)
        self.dec2 = DecoderBlockForUnet(256, 128, dropout_prob)
        self.dec1 = DecoderBlockForUnet(128, 64, dropout_prob)
        self.final_conv = nn.Conv2d(64, num_classes, kernel_size=1)
        self.activation = nn.Identity()

    def forward(self, x):
        encoded, (s1, s2, s3, s4, s5, s6) = self.encoder(x)
        x = self.dec6(encoded, s6)
        x = self.dec5(x, s5)
        x = self.dec4(x, s4)
        x = self.dec3(x, s3)
        x = self.dec2(x, s2)
...
# autoencoder.summary((in_channels_variable, 256, 256))

# unet = Unet(num_classes=1)
# unet.summary((in_channels_variable, 256, 256))
