...
subset_file = "/home/egmelich/SatelliteMAE/Autoencoder_Unet_Satellietdataportaal_Biesbosch/subset_indices_per_split_SDP_1024.json"
...
output_dir = "/home/egmelich/SatelliteMAE/Autoencoder_Unet_Satellietdataportaal_Biesbosch/saved_images_SDP_1024_final"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Ensure the models directory exists
models_dir = "/home/egmelich/SatelliteMAE/Autoencoder_Unet_Satellietdataportaal_Biesbosch/models_SDP_1024_final"
if not os.path.exists(models_dir):
    os.makedirs(models_dir)

# Load data for U-net training
train_images_dir = '/projects/0/prjs1235/Satellietdataportaal_data/Biesbosch_ManualAnnotation/Biesbosch_trainvaltest_SDP_1024/Biesbosch_images_train'
train_masks_dir = '/projects/0/prjs1235/Satellietdataportaal_data/Biesbosch_ManualAnnotation/Biesbosch_trainvaltest_SDP_1024/Biesbosch_masks_train'
val_images_dir = '/projects/0/prjs1235/Satellietdataportaal_data/Biesbosch_ManualAnnotation/Biesbosch_trainvaltest_SDP_1024/Biesbosch_images_val'
val_masks_dir =  '/projects/0/prjs1235/Satellietdataportaal_data/Biesbosch_ManualAnnotation/Biesbosch_trainvaltest_SDP_1024/Biesbosch_masks_val'
test_images_dir = '/projects/0/prjs1235/Satellietdataportaal_data/Biesbosch_ManualAnnotation/Biesbosch_trainvaltest_SDP_1024/Biesbosch_images_test'
test_masks_dir =  '/projects/0/prjs1235/Satellietdataportaal_data/Biesbosch_ManualAnnotation/Biesbosch_trainvaltest_SDP_1024/Biesbosch_masks_test'
...
#     def __init__(self, num_classes, smooth=1e-6, ignore_index=-1):
#         """
#         Weighted Dice Loss with dynamic class weights based on the number of pixels per class.
...
#     def forward(self, outputs, targets):
#         # Move outputs to probability space using softmax
#         outputs = F.softmax(outputs, dim=1)
...
#         pixel_counts = targets_one_hot.sum(dim=2)
#         pixel_weights = 1.0 / (pixel_counts + self.smooth)  # Avoid division by zero

#         # Normalize weights
#         pixel_weights = pixel_weights / pixel_weights.sum(dim=1, keepdim=True)

#         # Calculate the weighted Dice loss
#         weighted_dice_loss = (pixel_weights * (1 - dice_score)).sum(dim=1).mean()
...
# Check if GPU is available and which GPU is being used
...
# criterion = DynamicWeightedDiceLoss(num_classes=NUM_CLASSES, smooth=1e-6, ignore_index=-1).to(device)
# optimizer = optim.Adam(unet.parameters(), lr=0.0001, weight_decay=1e-4)
optimizer = torch.optim.AdamW(unet.parameters(), lr=0.0001, weight_decay=1e-4)
...
log_dir = f'/home/egmelich/SatelliteMAE/Autoencoder_Unet_Satellietdataportaal_Biesbosch/runs_SDP_1024_final/{EPOCHS}epochs_{SUBSET}subset_train_unet_{config}_{current_time}'
writer = SummaryWriter(log_dir=log_dir)

# Load pre-trained encoder weights
if config == 'pretrain_SDP':
    pretrained_encoder_weights = torch.load('/home/egmelich/SatelliteMAE/Autoencoder_Unet_Satellietdataportaal/models/pretrained_encoder_weights.pth', weights_only=True) 
...
#     # Print confusion matrix and per-class accuracy for the test set
...
#     plot_confusion_matrix(conf_matrix_test, class_names, 'Test Confusion Matrix', os.path.join(output_dir, f'test_conf_matrix_{config}.png'))
...
# final_model_path = '/home/egmelich/SatelliteMAE/Autoencoder_Unet_Satellietdataportaal_Biesbosch/final results/pretrained/unet_model_weights_50epochs_100subset_pretrain.pth'
...
#     # Adjust layout
#     plt.tight_layout(pad=1.0, w_pad=0.1)

#     # Save individual image
#     output_path = os.path.join(output_dir, f"unet_results_image_{i + 1}_{EPOCHS}epochs_{SUBSET}%subset_{config}.png")
...
# Visualize and save results for multiple test images
num_images_to_visualize = 30  # Number of images to visualize
...
while visualized_images < num_images_to_visualize:
    ...
    raw_test_image = raw_image.astype(np.float32)[:, :, :3]
    # raw_test_image = (raw_test_image - raw_test_image.min()) / (raw_test_image.max() - raw_test_image.min())
    raw_test_image = contrast_stretch(raw_test_image)
