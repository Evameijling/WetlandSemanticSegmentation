...
output_dir = "/home/egmelich/SatelliteMAE/Autoencoder_Unet_Sentinel/saved_images_finalunet300epochs"
...
criterion = DiceLoss()  
optimizer = optim.AdamW(unet.parameters(), lr=0.001, weight_decay=1e-4)
...
if config == 'pretrain':
    pretrained_encoder_weights = torch.load('/home/egmelich/SatelliteMAE/Autoencoder_Unet_Sentinel/models/pretrained_encoder_weights.pth', weights_only=True) 
...
for i in range(n_images):
    ...
    with torch.no_grad():
        outputs = unet(test_img_input).squeeze(0)  # shape: (C, H, W)
        probabilities = torch.softmax(outputs, dim=0).cpu().numpy()  # probabilities per class
        prediction = np.argmax(probabilities, axis=0)                # predicted class per pixel
        predicted_prob = np.max(probabilities, axis=0)               # probability of chosen class
    ...
    # Prepare RGB for display (taking first 3 channels, reorder to BGR -> RGB if needed)
    test_img_rgb = test_img[:3].permute(1, 2, 0).cpu().numpy()
    test_img_rgb = test_img_rgb[:, :, [2, 1, 0]]  # flipping channels if they're in BGR order
    test_img_rgb = contrast_stretch(test_img_rgb)
    ...
    # Colorbar for correctness map
    cbar = fig.colorbar(im_corr, ax=axes[3], fraction=0.046, pad=0.04)
    cbar.set_label('Certainty', rotation=90)

    # Legend for correctness scale
    correct_patch = mpatches.Patch(color='green', label='Correctly classified pixels')
    incorrect_patch = mpatches.Patch(color='red', label='Incorrectly classified pixels')
    fig.legend(
    ...
    # Save each figure
    output_path = os.path.join(output_dir, f"unet_results_{i}_{EPOCHS}epochs_{SUBSET}%subset_{config}.png")
    plt.savefig(output_path, bbox_inches='tight')
    print(f"Saved figure {i+1}/{n_images} to {output_path}")
    plt.close(fig)
